...
var _PORT = 8096; //Http port Node.js server will be listening on. Make sure that this is an open port and its the same as the one defined in MT4 indicator/EA.
...
http.createServer(function onRequest(request, response) {
	...
	request.addListener("end", function() {
		...
		ms = content[0].toString();

		if(ms != "")
		{
			var msg = ms;	//Parse the ms into string
